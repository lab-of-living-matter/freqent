...
parser.add_argumnet('--nCompartments', '-K', type=int, default=64,
...
trajs = np.zeros((args.nSim, args.nCompartments, 2, args.n_t_points))
...
for ii in range(args.nSim):
    traj = result[ii][0]
    ep = result[ii][1]
    ax_traj[0].pcolorfast(np.arange(), t_points, traj[:, 0, :], cmap='Reds')
    ax_traj[1].pcolorfast(np.arange(), t_points, traj[:, 1, :], 'k', alpha=0.2)
