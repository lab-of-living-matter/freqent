...
fig, ax = plt.subplots()
ax.plot(mu[np.logical_and(mu >= 0, mu <= 8)], kmax_mean[np.logical_and(mu >= 0, mu <= 8)], 'ko')
# ax.errorbar(mu[np.logical_and(mu >= 0, mu <= 8)], kmax_mean[np.logical_and(mu >= 0, mu <= 8)], yerr=kmax_std[np.logical_and(mu >= 0, mu <= 8)], fmt='ko')
ax.fill_between(mu[np.logical_and(mu >= 0, mu <= 8)],
                kmax_mean[np.logical_and(mu >= 0, mu <= 8)] - kmax_std[np.logical_and(mu >= 0, mu <= 8)],
                kmax_mean[np.logical_and(mu >= 0, mu <= 8)] + kmax_std[np.logical_and(mu >= 0, mu <= 8)],
                color='k', alpha=0.5)
