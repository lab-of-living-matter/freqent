...
pre_hopf_mean = np.mean(np.abs(kmax[alphas < hopf]), axis=1)
pre_hopf_std = np.std(np.abs(kmax[alphas < hopf]), axis=1)

# after the bifurcation, distribution is unimodal, so able to be described by a mean
post_hopf_mean = np.mean(kmax[alphas > hopf], axis=1)
post_hopf_std = np.std(kmax[alphas > hopf], axis=1)
...
ax.plot(alphas[alphas >= 1], kmax_mean[alphas >= 1], 'ko')
ax.fill_between(alphas[alphas >= 1], kmax_mean[alphas >= 1] - kmax_std[alphas >= 1], kmax_mean[alphas >= 1] + kmax_std[alphas >= 1],
                color='k', alpha=0.5)
ax.plot([hopf, hopf], [-0.05, np.pi], 'r--', label=r'$\alpha_{Hopf}$')
ax.tick_params(which='both', direction='in')
ax.set(xlabel=r'$\alpha$', ylabel=r'$\mathrm{argmax}_{q} \; \dot{s}$')
