...
bins = np.linspace(-2*np.pi, 2*np.pi, 50)
...
for file in os.listdir(os.path.join(parentDir, 'thermal')):
    with h5py.File(os.path.join(parentDir, 'thermal', file), 'r') as f:
        [n, _, _] = ax[0].hist(np.ravel(f['data']['anglemat'][:] - np.nanmean(f['data']['anglemat'])),
                               histtype='step',
                               color='k',
                               density=True,
                               bins=bins,
...
for file in os.listdir(os.path.join(parentDir, 'noncontractile')):
    with h5py.File(os.path.join(parentDir, 'noncontractile', file), 'r') as f:
        [n, _, _] = ax[1].hist(np.ravel(f['data']['anglemat'][:] - np.nanmean(f['data']['anglemat'])),
                               histtype='step',
                               color='k',
                               density=True,
                               bins=bins,
...
ax[1].set(xlabel=r'$\theta - \langle \theta \rangle$', ylabel='pdf')
ax[0].set(ylabel='pdf', title='Nematic director angle distribution')

legend_elements = [mpl.lines.Line2D([0], [0], color='k', linewidth=2, label='data'),
                   mpl.lines.Line2D([0], [0], color='r', linewidth=2, label='mean')]

ax[0].legend(handles=legend_elements, loc='lower center')
ax[1].legend(handles=legend_elements, loc='lower center')

plt.tight_layout()

fig.savefig(os.path.join(parentDir, datetime.today().strftime('%y%m%d') + '_angleDist.pdf'), format='pdf')
