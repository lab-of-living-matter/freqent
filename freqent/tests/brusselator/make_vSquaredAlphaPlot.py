...
ax.loglog(alphas, epr, 'o', label=r'$\dot{S}_{true}$')
ax.loglog(alphas, epr_blind, 'o', label=r'$\dot{S}_{blind}$')
ax.loglog(alphas, np.mean(epr_spectral, axis=1), 'ko', label=r'$\hat{\dot{S}}$')
ax.fill_between(alphas[np.argsort(alphas)],
                ...
                np.mean(epr_spectral, axis=1)[np.argsort(alphas)] - np.std(epr_spectral, axis=1)[np.argsort(alphas)],
                color='k', alpha=0.5)
ax.loglog(alphas, vsqs, 'o', color='C3', label=r'$\dot{S}_{\int v^2}$', alpha=0.7)
