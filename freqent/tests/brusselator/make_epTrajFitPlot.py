...
ax.set(xlabel='t', ylabel=r'$\Delta S_{blind}$', title=r'$\alpha = 5.96$')
...
fig.savefig(os.path.join(savepath, datetime.now().strftime('%y%m%d') + '_ep_blind_traj_alpha5.96.pdf'), format='pdf')
fig.savefig(os.path.join(savepath, datetime.now().strftime('%y%m%d') + '_ep_blind_traj_alpha5.96.png'), format='png')
